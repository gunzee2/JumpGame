...
public class PlayerController : MonoBehaviour
{
    ...
    private STEP step = STEP.NONE;              // Playerの現在の状態(次の状態から置き換えられる).
    private STEP next_step = STEP.NONE;         // Playerの次の状態(プレイヤーの状態が変化した時置き換えられる).

    private float highest_position_y;           // ジャンプ前の標高.
    private float step_timer = -1.0f;           // 状態変化までの経過時間(-1.0f == OFF, 0.0f以上 == ON).
    private float jump_timer = -1.0f;           // ボタンダウン後の経過時間(ジャンプ強弱をつけるため。-1.0f == OFF, 0.0f以上 == ON).
    ...
    void Update()
    {
        ...
        // 2.地面に着地していて、現在の状態がジャンプ状態(ちょうど着地したところ) => 走行状態へ遷移.
        // 3.上記以外の状態の場合 => 次の状態は 何もなし のまま.
        if (this.next_step == STEP.NONE && this.isGrounded)
        {
            switch (this.step)
            {
                case STEP.RUN:
        ...
        if(this.next_step != STEP.NONE)
        {
            ...
            {
                this.audio.PlayOneShot(jumpSound);  							// ジャンプ音を再生.
                new_velocity.y = Mathf.Sqrt(2.0f * 9.8f * JUMP_HEIGHT_MAX);		// ジャンプの高さからジャンプの初速を計算し、更新後の速度に反映.
                this.is_velocity_reduced = false;                               // 減速済みフラグ をクリアする.
            }
        }
        ...
        if (this.step == STEP.JUMP)
        {
            ...
            if (!this.is_button_pressing && !this.is_velocity_reduced && new_velocity.y > 0.0f)
            {
                ...
                this.is_velocity_reduced = true;			// 減速済みフラグをONにする(二重に減速させてしまうことを防ぐため).
            }
        }
    ...
    private void getMouseInput()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            this.is_button_pressing = true; 	// ボタン押下フラグをON.
        }
